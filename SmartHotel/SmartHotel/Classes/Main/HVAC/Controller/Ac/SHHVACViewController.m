...
/// 空调
@property (nonatomic, strong) SHHVAC *ac;
...
- (void)analyzeReceiveData:(NSNotification *)notification {
    ...
    if ((subNetID != self.ac.subnetID) || (deviceID != self.ac.deviceID)) {
        return;
    }
...
- (void)controlHVACSelectButton:(UIButton *)selectModelButton acModeValue:(SHAirConditioningModeKind)modeKind setTempertureKind:(Byte)tempertureKind temperture:(Byte )temperature {
    ...
    [[SHUdpSocket shareSHUdpSocket] sendDataWithOperatorCode:0XE3D8 targetSubnetID:self.ac.subnetID targetDeviceID:self.ac.deviceID additionalContentData:[NSMutableData dataWithBytes:controlModelData length:sizeof(controlModelData)] remoteMacAddress:[SHUdpSocket getRemoteControlMacAddress] needReSend:YES];
    ...
    [[SHUdpSocket shareSHUdpSocket] sendDataWithOperatorCode:0XE3D8 targetSubnetID:self.ac.subnetID targetDeviceID:self.ac.deviceID additionalContentData:[NSMutableData dataWithBytes:controlTempertureData length:sizeof(controlTempertureData)] remoteMacAddress:[SHUdpSocket getRemoteControlMacAddress] needReSend:YES];
...
- (IBAction)powerButtonClick {
    ...
    [[SHUdpSocket shareSHUdpSocket] sendDataWithOperatorCode:0XE3D8 targetSubnetID:self.ac.subnetID targetDeviceID:self.ac.deviceID additionalContentData:[NSMutableData dataWithBytes:controlData length:sizeof(controlData)] remoteMacAddress:[SHUdpSocket getRemoteControlMacAddress] needReSend:YES];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // 2.读取空调的温度范围
    [[SHUdpSocket shareSHUdpSocket] sendDataWithOperatorCode:0X1900 targetSubnetID:self.ac.subnetID targetDeviceID:self.ac.deviceID additionalContentData:nil remoteMacAddress:[SHUdpSocket getRemoteControlMacAddress] needReSend:NO];
    
    [NSThread sleepForTimeInterval:0.1];
    
    // 3.读取状态空调的开关状态
    [[SHUdpSocket shareSHUdpSocket] sendDataWithOperatorCode:0XE0EC targetSubnetID:self.ac.subnetID targetDeviceID:self.ac.deviceID  additionalContentData:[NSMutableData dataWithBytes:readHVACdata length:sizeof(readHVACdata)] remoteMacAddress:[SHUdpSocket getRemoteControlMacAddress] needReSend:NO];
}
